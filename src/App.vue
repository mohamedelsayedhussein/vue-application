<template>
  <div class="text-primary" id="root-component">
    ...
    <div>
      ...
      :key="profile.id"/>
      <button v-if="profiles && profiles.length < totalCount"
...
// <router-view /> is  builtin component tell to Vue-router please load the components routes in this place and show them
// يعنى بالبلدى الراوتر فيو بيعرض الكومبوننت اللى الراوت بتاعه معروض فى اليو ار ال
...
export default {
  ...
  methods: {
    getProfiles(page) {
      axios.get(`http://localhost:3000/profile?_page=${page}&_limit=2`)
      .then(response => {
        this.profiles = this.profiles ? this.profiles.concat(response.data) : response.data;
